          ...
          <div className="hidden md:flex items-center gap-3">
            ...
            {user && (
              <Link href="/cart" aria-label="Cart" className="p-2 rounded-2xl border border-beige hover:border-terracotta relative">
                ...
                {items.length > 0 && <span className="absolute -top-1 -right-1 bg-terracotta text-white text-[10px] w-4 h-4 rounded-full flex items-center justify-center">{items.length}</span>}
              </Link>
            )}
            <AuthButton />
          </div>
      ...
      {open && (
        <div className="md:hidden border-t border-beige bg-offwhite">
          <nav className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-3 grid gap-2 text-sm">
            <Link href="/catalog" className="py-2">Books</Link>
            <Link href="/club" className="py-2">Book Club</Link>
            <Link href="/events" className="py-2">Events</Link>
            <Link href="/consulting" className="py-2">Consulting</Link>
            {user && <Link href="/cart" className="py-2 flex justify-between">Cart {items.length > 0 && <span className="bg-terracotta text-white px-2 rounded-full text-xs">{items.length}</span>}</Link>}
